...
@ai_page.route("/analyze/init/", methods=["POST"])
@login_required
def init_analysis_session():
    """初始化多文件分析会话（数据库持久化）"""
    user_id = g.current_user["uuid"]
    try:
        data = request.get_json() or {}
        use_type = data.get("use", "personal")
        title = data.get("title", "文件分析会话")
        
        if use_type not in {"personal", "company"}:
            return jsonify({"code": 400, "message": "use 参数必须是 personal 或 company"})
        
        # analysis_type 字段用于区分分析会话
        conversation = create_conversation(user_id, "deepseek", title, analysis_type=use_type)
...
@ai_page.route("/analyze/<session_id>/upload/", methods=["POST"])
@login_required
def upload_to_analysis_session(session_id):
    """上传文件到分析会话（数据库持久化）"""
    user_id = g.current_user["uuid"]
    try:
        conversation = get_conversation(session_id, user_id)
        if not conversation:
            return jsonify({"code": 404, "message": "分析会话不存在"})
        
        # 只允许分析会话
        if conversation.get("analysis_type") not in {"personal", "company"}:
            return jsonify({"code": 400, "message": "不是分析会话"})
        
        files = request.files.getlist("files")
        if not files:
            return jsonify({"code": 400, "message": "未选择文件"})
        
        saved_files = conversation.get("files", []) or []
        file_details = conversation.get("file_details", []) or []
        base_dir = os.path.expanduser(PRODUCT_IMAGE_DIR)
        ...
        for f in files:
            ...
            ok_ext, ext_or_err = validate_file_ext(f.filename)
            if not ok_ext:
                return jsonify({"code": 400, "message": f"文件 {f.filename}: {ext_or_err}"})
            
            ok_size, size_or_err = validate_file_size(f)
            if not ok_size:
                return jsonify({"code": 400, "message": f"文件 {f.filename}: {size_or_err}"})
            
            file_ext = os.path.splitext(f.filename)[1]
            filename = f"{uuid.uuid4().hex}{file_ext}"
            path = os.path.join(base_dir, "chat-uploads", session_id, filename)
            dir_path = os.path.dirname(path)
        ...
        # 更新数据库
        update_conversation(session_id, conversation.get("messages", []), files=saved_files, title=conversation.get("title"), file_details=file_details)
...
@ai_page.route("/analyze/<session_id>/execute/", methods=["POST"])
@login_required
def execute_analysis(session_id):
    """执行分析（数据库持久化）"""
    user_id = g.current_user["uuid"]
    try:
        conversation = get_conversation(session_id, user_id)
        ...
        files_list = conversation.get("files", []) or []
        if not files_list:
            return jsonify({"code": 400, "message": "会话中没有文件"})
        ...
        analysis_prompt = get_analysis_prompt(conversation.get("analysis_type"), custom_instruction)
        ...
        def generate():
            try:
                ...
                yield f"event: start\ndata: {json.dumps({'session_id': session_id, 'file_count': len(files_list), 'use': conversation.get('analysis_type'), 'status': 'started'})}\n\n"
                
                response_text = ""
                for chunk in chat_func(messages=messages, files=files_list if files_list else None):
                ...
                update_conversation(session_id, messages, files_list, conversation.get("title"), conversation.get("file_details", []))
                
                yield f"event: end\ndata: {json.dumps({'status': 'completed', 'conversation_id': session_id})}\n\n"
            except Exception as e:
        ...
        return jsonify({"code": 500, "message": f"执行分析失败: {str(e)}"})


@ai_page.route("/analyze/<session_id>/continue/", methods=["POST"])
@login_required
def continue_analysis(session_id):
    ...
    model = conversation.get("model", "deepseek")
    messages = conversation.get("messages", []) or []
    files_list = conversation.get("files", []) or []
...
@ai_page.route("/analyze/<session_id>/", methods=["GET"])
@login_required
    ...
    try:
        ...
        return jsonify({
            ...
            "data": {
                ...
                "use": conversation.get("analysis_type"),
                "title": conversation.get("title"),
                "files": conversation.get("file_details", []),
                "messages": conversation.get("messages", []),
                "created_at": conversation.get("created_at"),
                "updated_at": conversation.get("updated_at")
            }
        ...
        return jsonify({"code": 500, "message": f"获取会话信息失败: {str(e)}"})


@ai_page.route("/analyze/<session_id>/", methods=["DELETE"])
@login_required
def delete_analysis_session(session_id):
    ...
    try:
        ...
        # 删除会话中的文件
        for file_path in conversation.get("files", []):
            try:
                if os.path.isfile(file_path):
                    os.remove(file_path)
            except Exception:
        ...
        # 删除上传目录
        base_dir = os.path.expanduser(PRODUCT_IMAGE_DIR)
        upload_dir = os.path.join(base_dir, "chat-uploads", session_id)
        if os.path.isdir(upload_dir):
            try:
                shutil.rmtree(upload_dir)
            except Exception:
        ...
        # 删除数据库中的对话
        delete_conversation(session_id, user_id)
