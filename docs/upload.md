...
| 参数 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `path` | String | 是 | 要删除的文件或目录相对路径（相对于上传基目录），例如 `ai-chat/user-uuid` 或 `avatar/user-uuid.jpg` |
...
**普通用户**：
- 仅可删除以下路径：
  1. `/ai-chat/<用户UUID>/` - 该用户在 AI 聊天中上传的所有文件和子目录
  2. `/avatar/<用户UUID>.{扩展名}` - 该用户的头像文件（仅限图片格式：jpg、png、gif、bmp、webp、svg、ico）

#### 请求示例

**示例 1：普通用户删除自己的 AI 聊天文件目录**

```bash
curl -X DELETE https://api.zhongrongxinfu.cn/upload/delete/ \
  -H "Authorization: Bearer your_token" \
  -H "Content-Type: application/json" \
  -d '{"path": "ai-chat/12345678-1234-1234-1234-123456789abc"}'
```
...
curl -X DELETE https://api.zhongrongxinfu.cn/upload/delete/ \
  -H "Authorization: Bearer your_token" \
  -H "Content-Type: application/json" \
  -d '{"path": "avatar/12345678-1234-1234-1234-123456789abc.jpg"}'
```
...
curl -X DELETE https://api.zhongrongxinfu.cn/upload/delete/ \
  ...
  -H "Content-Type: application/json" \
  -d '{"path": "product/electronics/item-001.jpg"}'
```
...
A: 使用 `DELETE /upload/delete/` 接口可以删除文件或目录。管理员可以删除任何路径，普通用户只能删除自己的 AI 聊天文件目录（`/ai-chat/<用户UUID>/`）和头像文件（`/avatar/<用户UUID>.jpg` 等图片格式）。详见上文的"删除文件或目录"接口说明。
...
A: 删除接口有严格的权限控制：
- **管理员**（`is_op=1`）：可以删除任何路径下的文件或目录，无任何限制
- **普通用户**：仅可删除以下两种路径：
  1. `/ai-chat/<用户UUID>/` - 自己在 AI 聊天中上传的文件（包含子目录中的所有文件）
  2. `/avatar/<用户UUID>.<扩展名>` - 自己的头像文件（仅限图片格式：jpg、jpeg、png、gif、bmp、webp、svg、ico）

### Q: 删除目录时会删除其中的所有文件吗？

A: 是的。删除目录接口会递归删除目录中的所有文件和子目录。响应中的 `deleted_items` 字段会显示实际删除的文件数量。

### Q: 删除文件是否可以恢复？

A: 不可以。删除操作是永久的，已删除的文件无法恢复。请谨慎操作。

### Q: 如果尝试删除没有权限的文件会怎样？

A: 接口会返回 403 错误，错误信息为"权限不足，无法删除该路径"。删除操作不会执行。
...
| 1.2 | 2026-01-24 | 添加文件删除接口，支持删除单个文件和目录，包含严格的权限控制（管理员无限制，普通用户仅可删除自己的 AI 聊天文件和头像） |
